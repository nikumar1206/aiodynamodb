...
| `hash_key_types` | `dict[Any, str]` | Built-in map | Mapping from Python type to DynamoDB attribute type (`"S"`, `"N"`, `"B"`). The built-in map includes `str`, `int`, `float`, `bytes`, `datetime`, timestamp custom types, `IntEnum`, and `StrEnum`. Override to add custom key types. |
